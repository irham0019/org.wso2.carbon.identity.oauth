...
public class JDBCScopeValidator extends OAuth2ScopeValidator {
    ...
    public boolean validateScope(OAuthTokenReqMessageContext tokReqMsgCtx) throws
        ...
        setValidScope(tokReqMsgCtx);
        log.debug("------call setValidScope validateScope(tokReqMsgCtx.getScope(), tokReqMsgCtx.getAuthorizedUser())");
    ...
    public boolean validateScope(OAuthAuthzReqMessageContext authzReqMessageContext) throws
            UserStoreException, IdentityOAuth2Exception {
        setValidScope(authzReqMessageContext);
        log.debug("------ call setValidScope validateScope(authzReqMessageContext.getAuthorizationReqDTO().getScope");
     ...
     * Validate given set of scopes against an authenticated user.
     *
     * @param tokReqMsgCtx Scopes to be validated.
     * @return True is all scopes are valid. False otherwise.
     * @throws UserStoreException If were unable to get tenant or user roles.
     * @throws IdentityOAuth2Exception by an Underline method.
     */
    private boolean setValidScope(OAuthTokenReqMessageContext tokReqMsgCtx)
            throws UserStoreException, IdentityOAuth2Exception {
        ...
        log.debug("call setValidScope validateScope(authzReqMessageContext.getAuthorizationReqDTO().getScopes(),\n" +
                "                authzReqMessageContext.getAuthorizationReqDTO().getUser())");
        // Remove OIDC scopes from the list if exists.
        try {
            String[] oidcScopes = OAuth2ServiceComponentHolder.getInstance().getOAuthAdminService().getScopeNames();
        ...
        scopeArray = scopes.toArray(scopeArray);
     ...
     * Validate given set of scopes against an authenticated user.
     *
     * @param authzReqMessageContext Scopes to be validated.
     * @return True is all scopes are valid. False otherwise.
     * @throws UserStoreException If were unable to get tenant or user roles.
     * @throws IdentityOAuth2Exception by an Underline method.
     */
    private boolean setValidScope(OAuthAuthzReqMessageContext authzReqMessageContext)
            throws UserStoreException, IdentityOAuth2Exception {
        ...
        for (String scope : requestedScopes) {
            ...
            if (!isUserAuthorizedForScope(scope, userRoles, tenantId)) {
                log.error("Requested scope " + scope + " is not UserAuthorizedForScope");
            } else {
                scopes.add(scope);
            }
        }
        String[] scopeArray = new String[scopes.size()];
        scopeArray = scopes.toArray(scopeArray);
        authzReqMessageContext.getAuthorizationReqDTO().setScopes(scopeArray);
        return true;
    }
